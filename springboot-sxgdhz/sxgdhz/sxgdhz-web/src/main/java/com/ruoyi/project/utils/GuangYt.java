...
public class GuangYt {
	private static Logger log = LoggerFactory.getLogger(GuangYt.class);
	
	public static void main(String[] args) {
		GuangYt sGuangYt =new GuangYt();
		sGuangYt.a();
	}
	...
	public void a() {
		...
		String guangUrl = CommonUtils.returnWebappsPath("wxYt/guang/")+smf.format(cal.getTime());
		//设置比时间少一个小时
		...
		loop1:for (int i = 0; i <= 8; i++) {
			...
			String parms = "userId=idc&pwd=U3cuYV&interfaceId=getVisibleCloudImage&dataFormat=xml2&ymdhms=["+timeIntervar+"]&cols=fileName,fileUrl,suffix,width,height,size,imgbase64";
			...
			try {
				...
				File file = new File(guangUrl);
				if (!file.exists() && !file.isDirectory()) {
					file.setWritable(true, false);
					file.mkdirs();
				}
				String fileaddress = guangUrl +file.separator+ filename;
				File fileExists = new File(fileaddress);
				if (fileExists.exists()) {
				 System.out.println("可见光云图已存在----》"+fileaddress);
				 continue loop1;
				}
